...
def generate_metadata_content(data: dict) -> str:
    # Функция для генерации txt файла
...
async def batch_download_and_send(message: types.Message, channel_url: str, limit: int):
    """
    Основная логика скачивания HD превью с защитой от ошибок на Render.
    """
    ...
    ydl_opts = {
        ...
        'no_warnings': True,
        'skip_download': True,
        # User-agent помогает избежать некоторых блокировок
    ...
    try:
        ...
        # --- ЗАЩИТА ОТ БЛОКИРОВКИ RENDER ---
        if info is None:
            await status_msg.edit_text("❌ Ошибка: YouTube не вернул данные (блокировка IP). Попробуйте позже.")
            return
        # -----------------------------------
        ...
        if total_found == 0:
            await status_msg.edit_text("❌ Видео не найдены. Возможно, канал пуст или требуется капча.")
            return
        ...
        for index, entry in enumerate(entries):
            ...
            try:
                ...
                if (is_size_limit or is_count_limit) and current_batch_files:
                    ...
                    if index % 50 == 0:
                        try: await status_msg.edit_text(f"📦 Обработано {index} из {total_found} (HD качество)...")
                        except: pass
...
async def run_video_analysis(message: types.Message, video_url: str, state: FSMContext):
    ...
    formatted_date = datetime.fromisoformat(data['published_at'].replace('Z', '+00:00')).strftime("%d.%m.%Y %H:%M:%S")
    ...
    if len(safe_desc) > 800: safe_desc = safe_desc[:800] + "... (читать по ссылке)"

    tags = data.get('tags', [])
    safe_tags = html.escape(", ".join(tags)) if tags else "Теги не найдены"
    if len(safe_tags) > 500: safe_tags = safe_tags[:500] + "..."
    ...
    if geo_info: lines.append(f"├ ГЕО: {geo_info}")
    
    lines.append(
        f"└ 👀: {format_number(data.get('views', 0))} │ "
        f"👍: {format_number(data.get('likes', 0))} │ "
        f"👎: {format_number(dislikes_count)} │ "
...
async def run_channel_analysis(message: types.Message, channel_input: str, state: FSMContext):
    ...
    if 'avg_views' in data:
        ...
        buttons.append(types.InlineKeyboardButton(text="📊 График активности", callback_data=f"show_graphs:{data['channel_id']}"))

    buttons.append(types.InlineKeyboardButton(text="📅 Теплокарта публикаций", callback_data=f"show_heatmap:{data['channel_id']}"))
...
async def process_thumb_channel_step(message: types.Message, state: FSMContext):
    ...
    await msg.delete()
    await message.answer(f"✅ Канал: <b>{html.escape(channel_data['title'])}</b>\n📹 Видео: {total_videos}\n🔢 <b>Сколько скачать? (от 1 до {total_videos})</b>", parse_mode="HTML")
...
async def cb_show_heatmap(cb: types.CallbackQuery):
    ...
    if not data.get("error"):
        ...
        if buf: await cb.message.answer_photo(BufferedInputFile(buf.getvalue(), filename="heatmap.png"), caption=data['report'], parse_mode="HTML")
...
async def process_niche_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Файл '{message.text}' создан. Отправляйте каналы.", reply_markup=get_niche_analysis_keyboard())
